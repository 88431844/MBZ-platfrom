...
public class SmallDogAttributeClass {
	
	public static void main(String[] args){
		String filePath = "F:\\MBH\\数据标定\\小狗机器人\\人脸标注项目\\元数据\\元数据路径\\1批11.2.txt";
		String path = "F:/MBH/数据标定/小狗机器人/人脸标注项目/元数据/第一批数据/";
		String tagPath = "F:/MBH/数据标定/小狗机器人/人脸标注项目/元数据/第一批数据/标注数据3/";
		...
		for (int i = 0; i < valueList.size(); i++) {
			...
			String face = null;
			String hair = null; 
			String definition = fileNameList[7];//清晰度
			...
			if("-1".equals(fileNameList[6])){
				hair = "(-1)无法判别类";
			}else if("0".equals(fileNameList[6])){
				hair = "(0)秃头";
			}else if("1".equals(fileNameList[6])){
				hair = "(1)短发";
			}else if("2".equals(fileNameList[6])){
				hair = "(2)中发";
			}else if("3".equals(fileNameList[6])){
				hair = "(3)中长发";
			}else if("4".equals(fileNameList[6])){
				hair = "(4)长发";
			}else{
				hair = "无效头发长度";
			}
...
//			DownloadImgFromURL.downloadPicture(url, path+fileName);
			...
			contentList.add(eye);
			contentList.add(face);
			contentList.add(hair);
			contentList.add(definition);
			WriteStringOnImg.writeStringListToImg(path+fileName+".jpg", tagPath+fileName+".jpg",contentList,30);
		}
	}
}
